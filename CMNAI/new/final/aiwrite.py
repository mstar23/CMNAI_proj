...
class last():
    ...
    def data_storage(current_user):
        ...
        img = cv2.imread(f'{test_img_path_list[-1]}') # 불러올 절대 경로
        time.sleep(3)
        img1 = img.copy()
        ...
        for i in range(len(contours)):
            if len(contours[i]) == 4: # 4는 격자의 상하좌우 즉 격자를 의미
                i_list += [i]
        i_list.reverse()
        i_list = i_list[:-1]
        ...
        for i, j in zip(i_list, range(len(i_list))):
            ...
            scaled_img_np[(scaled_img_np >= 180)] = 255 # 검정색 비스무리 한 것은 검정으로
            scaled_img_np[(scaled_img_np < 180)] = 0 # 검정색이 아닌 것들은 화이트로

            result_img_data = Image.fromarray(scaled_img_np)  # NumPy array to PIL image

            result_img_data.save(f'{result_folder_name}{j}.png', 'png') # last data 에 들어가는 부분

        result_data = last.data_ready() # 모델에 들어갈 수 있는 형태의 데이터가 모여 있음
        ...
        return word, img_url, url_html # url_html : html 에 뿌려줄 경로

    def data_ready(): # 텐서 형태로 들어가야해서 데이터 준비
