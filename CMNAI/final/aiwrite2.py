...
class last2():
    ...
    def data_storage(current_user):
        ...
        url_html = "".join(img_url_list[img_url_list.index('/static'):])       # /static ~~~
        ...
        #     for f in glob(f"{origin_folder_name}*.png"):  # test_data 내 이미지 석제
        #         os.remove(f)
            # for f in glob(f"{origin_folder_name}5 *.png"):  # test_data 내 이미지 석제
            #     os.remove(f)
